...
const AdventurerContext = createContext<AdventurerContextProps | undefined>(undefined);
export function AdventurerProvider({children}: {children: ReactNode}) {
    ...
    const {adventurers, loading, error, addAdventurer, fetchAdventurers} = useAdventurers(); // wrap API calls and related state in the context provider



    return <AdventurerContext.Provider value={{activeAdventurer, setActiveAdventurer, adventurers, loading, error, addAdventurer, fetchAdventurers}}>
        {children}
...
export function useAdventurer() {
    ...
    if (!context) throw new Error("useAdventurer must be used within AdventurerProvider");
    return context;
}
